...
// https://syzkaller.appspot.com/bug?id=b673a563b3a339728b15
// status:0
...
void execute_one(void)
{
  ...
  res = -1;
  NONFAILING(res = syz_init_net_socket(0x1f, 1, 3));
  if (res != -1)
    r[0] = res;
  res = syscall(__NR_dup, r[0]);
  if (res != -1)
    r[1] = res;
  NONFAILING(*(uint16_t*)0x20000180 = 0x1f);
  NONFAILING(*(uint8_t*)0x20000182 = -1);
  ...
  NONFAILING(*(uint8_t*)0x20000187 = -1);
  NONFAILING(*(uint8_t*)0x20000188 = 9);
  syscall(__NR_connect, r[1], 0x20000180ul, 0xaul);
  syscall(__NR_setsockopt, -1, 6, 2, 0ul, 0ul);
