...
// https://syzkaller.appspot.com/bug?id=3e12e14ee01b675e1af2
// status:0
...
void loop(void)
{
  ...
  res = -1;
  res = syz_open_dev(0x20000000, 0x20, 0);
  if (res != -1)
    r[0] = res;
  syscall(__NR_fcntl, r[0], 4ul, 0x82400ul);
  memcpy((void*)0x20000000, "/dev/input/event#\000", 18);
  res = -1;
  res = syz_open_dev(0x20000000, 0x20, 0x101002);
  if (res != -1)
    r[1] = res;
  *(uint64_t*)0x200001c0 = 0x77359400;
  *(uint64_t*)0x200001c8 = 0;
  *(uint16_t*)0x200001d0 = 1;
  *(uint16_t*)0x200001d2 = 0x3a;
  *(uint32_t*)0x200001d4 = 2;
  syscall(__NR_write, r[1], 0x200001c0ul, 0x214ul);
  res = syscall(__NR_socket, 0x200000000000011ul, 3ul, 0);
  if (res != -1)
    r[2] = res;
  syscall(__NR_fcntl, r[2], 4ul, 0x2000ul);
  memcpy((void*)0x20000240, "ip6_vti0\000\000\000\000\000\000\000\000", 16);
  ...
  *(uint8_t*)0x20000053 = 0;
  syscall(__NR_bind, r[2], 0x20000040ul, 0x14ul);
  *(uint32_t*)0x200002c0 = 0x596;
  syscall(__NR_setsockopt, r[2], 1, 0x25, 0x200002c0ul, 4ul);
  syscall(__NR_sendmmsg, r[2], 0x20000d00ul, 0x400004eul, 0ul);
}
int main(void)
{
