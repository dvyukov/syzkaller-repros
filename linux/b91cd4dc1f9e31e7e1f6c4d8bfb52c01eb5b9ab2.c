...
// https://syzkaller.appspot.com/bug?id=b91cd4dc1f9e31e7e1f6c4d8bfb52c01eb5b9ab2
// status:open
...
{
  long res = 0;
  memcpy((void*)0x200008c0, "/dev/infiniband/rdma_cm\000", 24);
  res = syscall(__NR_openat, 0xffffffffffffff9c, 0x200008c0, 2, 0);
  if (res != -1)
    r[0] = res;
  ...
  *(uint8_t*)0x2000011f = 0;
  syscall(__NR_write, r[0], 0x20000100, 0x20);
  memcpy((void*)0x20000140, "\x15\x00\x00\x00\x08\x00\x00\x00\x00\x00\x00\x00"
                            "\xc8\x6b\x65\x25\x00\x00\x10",
         19);
  syscall(__NR_write, r[0], 0x20000140, 0x13);
  res = syscall(__NR_socket, 2, 1, 0);
  if (res != -1)
    r[1] = res;
  memcpy((void*)0x20000040, "syz_tun\000\000\000\000\000\000\000\000\000", 16);
  *(uint16_t*)0x20000050 = 2;
  *(uint16_t*)0x20000052 = htobe16(0x4e20);
  *(uint32_t*)0x20000054 = htobe32(0);
  syscall(__NR_ioctl, r[1], 0x8918, 0x20000040);
}
int main(void)
{
