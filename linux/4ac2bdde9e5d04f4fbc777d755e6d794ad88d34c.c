...
// https://syzkaller.appspot.com/bug?id=4ac2bdde9e5d04f4fbc777d755e6d794ad88d34c
// status:dup
...
{
  long res = 0;
  NONFAILING(memcpy((void*)0x20000000, "cpuset.memory_pressure", 23));
  res = syscall(__NR_openat, 0xffffff9c, 0x20000000, 0, 0);
  if (res != -1)
    r[0] = res;
  NONFAILING(memcpy((void*)0x20000040, "/dev/mixer", 11));
  res = syscall(__NR_openat, 0xffffffffffffff9c, 0x20000040, 0x40000, 0);
  if (res != -1)
    r[1] = res;
  NONFAILING(*(uint32_t*)0x20000180 = r[1]);
  NONFAILING(*(uint32_t*)0x20000184 = 3);
  NONFAILING(*(uint16_t*)0x20000188 = 0xfff);
  syscall(__NR_ioctl, r[0], 0x400442c8, 0x20000180);
  NONFAILING(memcpy((void*)0x20000140, "\x63\x51\x2d\x1c\x00", 5));
  NONFAILING(memcpy((void*)0x20000080, "/", 2));
  NONFAILING(memcpy((void*)0x200000c0, "noblock_validity", 16));
  NONFAILING(*(uint8_t*)0x200000d0 = 0x2c);
  NONFAILING(*(uint8_t*)0x200000d1 = 0);
  syz_mount_image(0x20000140, 0x20000080, 0, 0, 0x20000200, 0x28020,
                  0x200000c0);
}
