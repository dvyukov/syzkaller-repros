...
// https://syzkaller.appspot.com/bug?id=6e06cc8d0b140bd8fde4
// status:0
...
void execute_one(void)
{
  ...
  if (res != -1)
    r[0] = res;
  memcpy((void*)0x20000000, "veth0_to_bridge\000", 16);
  ...
  *(uint64_t*)0x20000018 = 0;
  syscall(__NR_ioctl, r[1], 0x89a2, 0x20000000ul);
  ...
  *(uint32_t*)0x20000168 = 0xffffffaf;
  *(uint16_t*)0x2000016c = 0;
  *(uint16_t*)0x2000016e = 0;
  *(uint32_t*)0x20000170 = 0;
  *(uint32_t*)0x20000174 = 0;
  *(uint64_t*)0x20000178 = 0;
  res = syscall(__NR_perf_event_open, 0x20000100ul, 0, -1ul, -1, 0ul);
  if (res != -1)
    r[2] = res;
  res = syscall(__NR_perf_event_open, 0ul, 0, 6ul, r[2], 1ul);
  if (res != -1)
    r[3] = res;
  syscall(__NR_read, r[3], 0ul, 0ul);
  syz_open_dev(0, 4, 0x202);
