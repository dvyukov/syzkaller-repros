...
// https://syzkaller.appspot.com/bug?id=b0e376d3731cd5e82bb7
// status:0
...
{
  intptr_t res = 0;
  NONFAILING(memcpy((void*)0x200000c0, "net/igmp\000", 9));
  res = syz_open_procfs(0, 0x200000c0);
  if (res != -1)
    r[0] = res;
  NONFAILING(memcpy((void*)0x20000840, "/dev/ttyS3\000", 11));
  res = syscall(__NR_openat, 0xffffffffffffff9cul, 0x20000840ul, 0x802ul, 0ul);
  if (res != -1)
    r[1] = res;
  syscall(__NR_sendfile, r[1], r[0], 0ul, 0x7ffff000ul);
  NONFAILING(memcpy((void*)0x20000640, "/dev/ttyS3\000", 11));
