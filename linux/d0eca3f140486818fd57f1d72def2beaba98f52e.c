// BUG: soft lockup in rcu_read_lock_held
// https://syzkaller.appspot.com/bug?id=d0eca3f140486818fd57f1d72def2beaba98f52e
// status:invalid
...
void execute_one()
{
  ...
  syscall(__NR_listen, r[0], 0xffffff7f);
  NONFAILING(*(uint32_t*)0x20000140 = 0);
  NONFAILING(*(uint32_t*)0x20000144 = 6);
  NONFAILING(*(uint32_t*)0x20000148 = 7);
  NONFAILING(*(uint32_t*)0x2000014c = 4);
  syscall(__NR_setsockopt, r[0], 0x84, 0, 0x20000140, 0x10);
  res = syscall(__NR_socket, 0xa, 1, 0x84);
  if (res != -1)
    r[1] = res;
  NONFAILING(memcpy((void*)0x20e33fe0, "X", 1));
  ...
  syscall(__NR_sendto, r[1], 0x20e33fe0, 1, 0, 0x2005ffe4, 0x1c);
}
