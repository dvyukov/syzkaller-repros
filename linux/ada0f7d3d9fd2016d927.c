...
// https://syzkaller.appspot.com/bug?id=ada0f7d3d9fd2016d927
// status:6
...
void execute_call(int call)
{
  ...
  case 0:
    ...
    res = -1;
    NONFAILING(res = syz_open_dev(0x20000000, 0x75, 0x281));
    if (res != -1)
      r[0] = res;
    break;
  case 1:
    syscall(__NR_ioctl, r[0], 0x80045518, 0ul);
    break;
  case 2:
    NONFAILING(*(uint32_t*)0x20000040 = 0);
    NONFAILING(*(uint32_t*)0x20000044 = 0x5516);
    NONFAILING(*(uint64_t*)0x20000048 = 0);
    syscall(__NR_ioctl, r[0], 0xc0105512, 0x20000040ul);
    break;
  case 3:
    NONFAILING(memcpy((void*)0x20000080, "trusted.overlay.opaque\000", 23));
    syscall(__NR_fsetxattr, r[0], 0x20000080ul, 0ul, 0ul, 1ul);
    break;
  case 4:
    NONFAILING(memcpy((void*)0x200000c0, "/dev/bus/usb/00#/00#\000", 21));
    res = -1;
    NONFAILING(res = syz_open_dev(0x200000c0, 0x75, 0x281));
    if (res != -1)
      r[1] = res;
    break;
  case 5:
    NONFAILING(STORE_BY_BITMASK(uint8_t, , 0x20000340, 0, 0, 7));
    NONFAILING(STORE_BY_BITMASK(uint8_t, , 0x20000340, 1, 7, 1));
    NONFAILING(*(uint32_t*)0x20000344 = 0);
    NONFAILING(*(uint32_t*)0x20000348 = 5);
    NONFAILING(*(uint64_t*)0x20000350 = 0);
    syscall(__NR_ioctl, r[1], 0xc0185502, 0x20000340ul);
    break;
  case 6:
    NONFAILING(STORE_BY_BITMASK(uint8_t, , 0x20000340, 1, 0, 7));
    NONFAILING(STORE_BY_BITMASK(uint8_t, , 0x20000340, 1, 7, 1));
    NONFAILING(*(uint32_t*)0x20000344 = 0x8b);
    NONFAILING(*(uint32_t*)0x20000348 = 0);
    NONFAILING(*(uint64_t*)0x20000350 = 0x20000280);
    NONFAILING(memcpy(
        (void*)0x20000280,
        "\xc2\x83\xe9\xf5\x7c\x99\x09\xfd\x96\x70\x76\xbb\xda\xda\x45\x2d\xa1"
        "\x62\x6d\xb4\x93\x1c\xc9\xea\x2c\xa5\xac\xbb\x7f\x07\x59\x7b\x26\x51"
        "\x75\x97\xd7\x62\x94\x6e\xc7\xdc\xf4\x9c\xd8\xb6\x71\x23\x76\x1c\x83"
        "\x28\x41\xdb\xce\x64\xa9\x4f\x4b\xe7\x41\x88\x61\x49\xca\x29\x3f\x1d"
        "\xe1\xe2\xc4\x37\x8e\x5f\x8d\xc9\xd6\x89\x89\x9b\x39\x43\x33\xd9\x3f"
        "\x22\x94\x8a\x06\x37\x47\xc1\x11\xa8\xeb\x94\x2c\xe2\x58\xf3\x93\x4e"
        "\x80\x76\x57\xb6\x06\x06\x55\x4f\xec\xb0\x3c\x13\xfb\xa5\x2f\x24\xf7"
        "\x1b\x50\x93\x5f\x72\xd3\x0b\xfb\x8e\xaf\x3e\x17\x0f\xbf\x7a\xfd\x19"
        "\xf6\x1b\x9a",
        139));
    syscall(__NR_ioctl, -1, 0xc0185502, 0x20000340ul);
    break;
  }
}
int main(void)
