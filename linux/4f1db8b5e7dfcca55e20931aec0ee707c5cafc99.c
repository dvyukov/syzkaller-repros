...
void execute_call(int call)
{
  ...
  switch (call) {
  case 0:
    NONFAILING(memcpy((void*)0x20000280, "/dev/kvm", 9));
  ...
  case 4:
    ...
    NONFAILING(memcpy((void*)0x20000000,
                      "\x44\x0f\x20\xc0\x35\x0f\x00\x00\x00\x44\x0f\x22\xc0\x0f"
                      "\x30\x66\xba\xf8\x0c\xb8\x20\x8c\x72\x83\xef\x66\xba\xfc"
                      "\x0c\xed\xf4\xf2\x36\x0f\x01\xdf\x64\x0f\x01\xd1\x0f\x85"
                      "\x02\x00\x00\x00\x66\xba\x40\x00\x66\xb8\x00\x20\x66\xef"
                      "\x66\x0f\x75\xfd\x65\x0f\x09",
                      63));
    NONFAILING(*(uint64_t*)0x20000050 = 0x3f);
    syz_kvm_setup_cpu(-1, r[2], 0x20001000, 0x20000040, 1, 0, 0x20000080, 0);
    break;
  case 5:
    NONFAILING(memcpy((void*)0x20000080, "/dev/snd/pcmC#D#c", 18));
    res = syz_open_dev(0x20000080, 6, 0x800);
    if (res != -1)
      r[3] = res;
    break;
  case 6:
    ...
    syscall(__NR_getsockopt, r[3], 0x103, 2, 0x200000c0, 0x20000100);
    break;
  case 7:
    res = syscall(__NR_fcntl, r[0], 9);
    if (res != -1)
      r[4] = res;
    break;
  case 8:
    syscall(__NR_sched_rr_get_interval, r[4], 0x20000140);
    break;
  case 9:
    NONFAILING(*(uint32_t*)0x200001c0 = 0xc);
    syscall(__NR_getsockopt, r[3], 0, 0x20, 0x20000180, 0x200001c0);
    break;
  case 10:
