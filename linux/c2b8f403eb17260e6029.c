...
// https://syzkaller.appspot.com/bug?id=c2b8f403eb17260e6029
// status:0
...
{
  intptr_t res = 0;
  memcpy((void*)0x20000000, "/proc/sys/net/ipv4/tcp_mtu_probing\000", 35);
  res = syscall(__NR_openat, 0xffffffffffffff9cul, 0x20000000ul, 1ul, 0ul);
  if (res != -1)
    r[0] = res;
  memcpy((void*)0x20000180, "5\000", 2);
  syscall(__NR_write, r[0], 0x20000180ul, 1ul);
  ...
  *(uint32_t*)0x20000144 = htobe32(-1);
  ...
  memcpy((void*)0x20000100, "cdg\000", 4);
  syscall(__NR_setsockopt, r[1], 6, 0xd, 0x20000100ul, 4ul);
  ...
  *(uint8_t*)0x20000087 = 0x12;
  syscall(__NR_sendto, r[1], 0ul, 0ul, 0x1000000020000000ul, 0x20000080ul,
          0x10ul);
  syscall(__NR_sendto, r[1], 0x20000240ul, 0xfffffd46ul, 0x81f4ul, 0ul,
          0x1f4ul);
}
int main(void)
{
