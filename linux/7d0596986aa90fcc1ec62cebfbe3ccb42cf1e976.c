...
// https://syzkaller.appspot.com/bug?id=7d0596986aa90fcc1ec62cebfbe3ccb42cf1e976
// status:open
...
void execute_one(void)
{
  ...
  *(uint32_t*)0x20000084 = htobe32(-1);
  syscall(__NR_bind, r[1], 0x20000080, 0x10);
  syscall(__NR_socket, 0x11, 0, 0x300);
  syscall(__NR_getsockopt, -1, 0x107, 0, 0, 0);
  syscall(__NR_perf_event_open, 0, 0, -1, -1, 0);
  syscall(__NR_socket, 0x11, 3, 0x300);
  syscall(__NR_clone, 0, 0, 0x9999999999999999, 0, -1);
  syscall(__NR_bind, -1, 0, 0);
  syscall(__NR_setsockopt, -1, 1, 0, 0, 0);
  res = syscall(__NR_socket, 0xa, 0, -1);
  if (res != -1)
    r[2] = res;
  syscall(__NR_setsockopt, r[0], 0, 0x48b, 0, 0);
  syscall(__NR_recvfrom, -1, 0, 0, 0, 0, 0);
  syscall(__NR_ioctl, -1, 0x540f, 0);
  syscall(__NR_ptrace, 3, 0, 0, 0);
  syscall(__NR_setsockopt, r[1], 1, 0x40000000000002e, 0, 0);
  syscall(__NR_openat, -1, 0, 2, 0);
  syscall(__NR_bind, r[2], 0, 0);
  syscall(__NR_getitimer, 0, 0);
  syscall(__NR_getpid);
  syscall(__NR_mknod, 0, 0, 0x701);
  ...
  *(uint8_t*)0x20000107 = 0xa;
  syscall(__NR_sendto, r[1], 0, 0, 0x20000802, 0x20000100, 0x10);
  memcpy((void*)0x20000000, "ip6_vti0\000\000\000\000\000\000\000\000", 16);
  syscall(__NR_setsockopt, r[1], 1, 0x19, 0x20000000, 0x10);
  syscall(__NR_sendto, r[1], 0, 0, 0, 0, 0);
  syscall(__NR_shutdown, r[1], 2);
}
int main(void)
{
