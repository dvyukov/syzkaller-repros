...
// https://syzkaller.appspot.com/bug?id=47665dbce263479409c8
// status:0
...
int main(void)
{
  ...
  NONFAILING(
      syz_io_uring_setup(0x45d8, 0x20000080, 0x20ee7000, 0x20ffd000, 0, 0));
  res = syscall(__NR_ioctl, r[0], 0xae01, 0ul);
  if (res != -1)
    r[1] = res;
  NONFAILING(*(uint64_t*)0x200023c0 = 8);
  NONFAILING(*(uint64_t*)0x200023c8 = 0);
  NONFAILING(*(uint64_t*)0x200023d0 = 0);
  NONFAILING(syz_kvm_setup_cpu(r[1], -1, 0x20fe8000, 0x200023c0, 1, 0, 0, 0));
  res = syscall(__NR_ioctl, r[1], 0xae41, 0ul);
  if (res != -1)
    r[2] = res;
  NONFAILING(*(uint64_t*)0x20000000 = 0x20);
  NONFAILING(*(uint64_t*)0x20000008 = 0x200002c0);
  NONFAILING(memcpy((void*)0x200002c0,
                    "\xb8\x05\x00\x00\x00\xb9\x26\xb7\x00\x00\x0f\x01\xd9\x66"
                    "\x0f\x38\x80\x24\x0f\x66\xba\x21\x00\xb8\xac\x42\x00\x00"
                    "\xef\x0f\x01\xca\x66\xba\xd0\x04\xec\xc4\xc3\x9d\x7e\x50"
                    "\xf1\x4b\x2e\x65\x0f\x01\xdf\x6f\xb9\x80\x00\x00\xc0\x0f"
                    "\x32\x35\x00\x80\x00\x00\x0f\x30\x64\x65\x0f\xc7\x6d\x07",
                    70));
  NONFAILING(*(uint64_t*)0x20000010 = 0x46);
  NONFAILING(
      syz_kvm_setup_cpu(-1, r[2], 0x20fe8000, 0x20000000, 1, 0x49, 0, 0));
