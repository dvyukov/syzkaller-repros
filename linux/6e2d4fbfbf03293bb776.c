...
// https://syzkaller.appspot.com/bug?id=6e2d4fbfbf03293bb776
// status:0 arch:386
...
{
  intptr_t res = 0;
  NONFAILING(memcpy((void*)0x20001440, "/dev/snd/controlC#\000", 19));
  res = syz_open_dev(0x20001440, 0, 0);
  if (res != -1)
    r[0] = res;
  NONFAILING(*(uint32_t*)0x20000040 = 0);
  syscall(__NR_ioctl, (intptr_t)r[0], 0x40045532, 0x20000040);
  NONFAILING(memcpy((void*)0x20000140, "/dev/audio\000", 11));
  res = syscall(__NR_openat, 0xffffff9c, 0x20000140, 0x40201, 0);
  if (res != -1)
    r[1] = res;
  NONFAILING(memcpy((void*)0x200001c0, "/dev/snd/pcmC#D#p\000", 18));
  res = syz_open_dev(0x200001c0, 0, 0xa2c65);
  if (res != -1)
    r[2] = res;
  NONFAILING(*(uint32_t*)0x20000240 = 0);
  NONFAILING(*(uint16_t*)0x20000244 = 0x18);
  ...
  syscall(__NR_write, (intptr_t)r[1], 0x20000240, 0x20);
  syscall(__NR_ioctl, (intptr_t)r[2], 0x4112, 0);
}
int main(void)
{
