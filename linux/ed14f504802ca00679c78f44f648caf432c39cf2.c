// BUG: soft lockup in mousedev_write
// https://syzkaller.appspot.com/bug?id=ed14f504802ca00679c78f44f648caf432c39cf2
// status:invalid
...
void execute_one()
{
  ...
  NONFAILING(*(uint8_t*)0x20000148 = 0xe4);
  ...
  NONFAILING(STORE_BY_BITMASK(uint64_t, 0x20000168, 0x10000003, 5, 1));
  ...
  NONFAILING(memcpy((void*)0x20660ff6, "/dev/ptmx", 10));
  syscall(__NR_openat, 0xffffffffffffff9c, 0x20660ff6, 0, 0);
  NONFAILING(memcpy((void*)0x20000000, "/dev/input/mice", 16));
  res = syz_open_dev(0x20000000, 0, 2);
  if (res != -1)
    r[1] = res;
  NONFAILING(memcpy((void*)0x20000040, "threaded", 9));
  syscall(__NR_write, r[1], 0x20000040, 0xffffffffffffff32);
}
