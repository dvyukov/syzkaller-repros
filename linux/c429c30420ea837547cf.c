...
// https://syzkaller.appspot.com/bug?id=c429c30420ea837547cf
// status:0
...
void loop(void)
{
  ...
  if (res != -1)
    r[0] = res;
  syscall(__NR_mmap, 0x20000000ul, 0x600000ul, 0ul, 0x10ul, r[0], 0ul);
  syscall(__NR_read, r[0], 0ul, 0ul);
  syscall(__NR_perf_event_open, 0ul, 0, 0xaul, r[0], 0ul);
  ...
  if (res != -1)
    r[1] = res;
  syscall(__NR_lseek, r[1], 0x800002ul, 0ul);
  syscall(__NR_write, r[1], 0x20000080ul, 0x8aul);
  res = syscall(__NR_socket, 0xaul, 0x400000000001ul, 0);
  if (res != -1)
    r[2] = res;
  syscall(__NR_close, r[2]);
  res = syscall(__NR_socket, 0x1eul, 4ul, 0);
  if (res != -1)
    r[3] = res;
  ...
  syscall(__NR_connect, r[3], 0x20000000ul, 0x10ul);
  NONFAILING(memcpy((void*)0x20002000, "./bus\000", 6));
  res = syscall(__NR_open, 0x20002000ul, 0ul, 0ul);
  if (res != -1)
    r[4] = res;
  inject_fault(39);
  syscall(__NR_sendfile, r[2], r[4], 0ul, 0x200fc0ul);
  close_fds();
}
int main(void)
{
