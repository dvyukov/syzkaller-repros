...
void loop(void)
{
  ...
  if (res != -1)
    r[0] = res;
  NONFAILING(*(uint32_t*)0x20000280 = 0xffffffc6);
  NONFAILING(*(uint8_t*)0x20000284 = 0x7b);
  NONFAILING(*(uint16_t*)0x20000285 = 0);
  syscall(__NR_write, r[0], 0x20000280ul, 0x1033bul);
  NONFAILING(memcpy((void*)0x20000200, "/dev/loop#\000", 11));
  res = syz_open_dev(0x20000200, 0, 0x182);
  if (res != -1)
    r[1] = res;
  ...
  syscall(__NR_pwritev, r[2], 0x20000340ul, 1ul, 0x81805ul);
  syscall(__NR_ioctl, r[1], 0x4c00ul, r[2]);
  syscall(__NR_sendfile, r[1], r[1], 0ul, 0x24000000ul);
  syscall(__NR_ioctl, r[1], 0x4c01ul, 0);
  NONFAILING(memcpy((void*)0x20000100, "./file0\000", 8));
  res = syscall(__NR_open, 0x20000100ul, 0ul, 0ul);
  if (res != -1)
    r[3] = res;
  syscall(__NR_ioctl, r[1], 0x4c00ul, r[3]);
  close_fds();
}
int main(void)
{
