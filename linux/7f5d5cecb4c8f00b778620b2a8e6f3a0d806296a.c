...
// https://syzkaller.appspot.com/bug?id=7f5d5cecb4c8f00b778620b2a8e6f3a0d806296a
// status:fixed
...
{
  syscall(__NR_socket, 0xful, 3ul, 2ul);
  NONFAILING(memcpy(
      (void*)0x200004c0,
      "\000\000\000\000\000\000\000\000\000\000\000\000\000\000\000\000", 16));
  NONFAILING(*(uint16_t*)0x200004d0 = 0x5002);
  syscall(__NR_ioctl, -1, 0x400454caul, 0x200004c0ul);
  syscall(__NR_io_setup, 5, 0x200002c0ul);
  NONFAILING(*(uint64_t*)0x20000540 = 0x200000c0);
  NONFAILING(*(uint64_t*)0x200000c0 = 0);
  NONFAILING(*(uint32_t*)0x200000c8 = 0);
  NONFAILING(*(uint32_t*)0x200000cc = 0x18);
  ...
  syscall(__NR_io_submit, 0ul, 1ul, 0x20000540ul);
  syscall(__NR_ioctl, -1, 0x800443d2ul, 0ul);
  ...
  syz_open_dev(0, 0x8001, 0);
  syscall(__NR_ioctl, -1, 0x4004ae86ul, 0ul);
  NONFAILING(memcpy((void*)0x200001c0, "./file0\000", 8));
  syscall(__NR_mount, 0x20000000ul, 0x200001c0ul, 0ul, 0x2001001ul, 0ul);
  NONFAILING(memcpy((void*)0x20000200, "./file0\000", 8));
  syz_mount_image(0, 0x20000200, 0, 0, 0, 0x100032, 0);
  syscall(__NR_mmap, 0x20000000ul, 0xff5000ul, 0ul, 0x10ul, -1, 0);
  ...
  syz_kvm_setup_cpu(-1, -1, 0x20cfc000, 0, 0, 0, 0, 0);
}
int main(void)
{
