...
// https://syzkaller.appspot.com/bug?id=db623516b5a108ea6784
// status:0
...
void loop(void)
{
  ...
  NONFAILING(memcpy(
      (void*)0x20000200,
      "\x18\x02\x00\x00\xf5\xfe\xff\xff\x00\x00\x00\x00\x00\x00\x00\x00\x85\x00"
      "\x00\x00\x07\x00\x00\x00\x95\x00\x00\x00\x00\x00\x00\x00\x67\xe8\xa2\xf1"
      "\x4d\x96\xa4\x7f\x09\x18\x28\x3a\xc8\x4d\xa4\x06\x00\x00\x00\x02\x68\xff"
      "\x3b\x53\x87\xec\x7a\x9c\x45\xff\x01\x00\x00\x00\x00\x00\x00\xfb\x84\xde"
      "\x35\xb5\x08\x6d\x51\x58\x47\xab\x12\x46\xcb\x03\x10\xff\xa8\x05\xbc\xab"
      "\x7f\x71\x48\xcf\xd6\x5a\x28\x6f\xff\x5d\x08\xdc\xdf\xcc\x0a\x61\xe9\xab"
      "\xb8\x25\x27\x6d\x85\x1e\x90\x3c\x9c\x33\x6e\x7d\xd4\x05\x00\x94",
      124));
  ...
  res = syscall(__NR_bpf, 5ul, 0x20000180ul, 0x30ul);
  if (res != -1)
    r[1] = res;
  ...
  NONFAILING(*(uint32_t*)0x20000000 = 0x40);
  ...
  NONFAILING(*(uint16_t*)0x20000020 = 0xc);
  NONFAILING(STORE_BY_BITMASK(uint16_t, , 0x20000022, 0x2b, 0, 14));
  ...
  NONFAILING(*(uint32_t*)0x20000028 = r[1]);
  NONFAILING(*(uint16_t*)0x2000002c = 0x14);
  NONFAILING(*(uint16_t*)0x2000002e = 3);
  NONFAILING(memcpy((void*)0x20000030, "veth0_to_bridge\000", 16));
