// KMSAN: kernel-infoleak in __kvm_write_guest_page (2)
// https://syzkaller.appspot.com/bug?id=8dba13dc49eee1170a4305ab9bd52101b0ac2505
// status:invalid
...
int main(void)
{
  ...
  intptr_t res = 0;
  NONFAILING(memcpy((void*)0x200001c0, "/dev/kvm\000", 9));
  ...
  NONFAILING(*(uint64_t*)0x200002d8 = 0x20000000);
  syscall(__NR_ioctl, r[1], 0x4020ae46, 0x200002c0);
  NONFAILING(memcpy((void*)0x20000000, "\x0f\xae", 2));
  syscall(__NR_ioctl, -1, 0xc008aeb0, 0x20000000);
  syz_kvm_setup_cpu(-1, r[2], 0x20028000, 0, 0, 0, 0, 0);
