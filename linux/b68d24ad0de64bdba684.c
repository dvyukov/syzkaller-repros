...
// https://syzkaller.appspot.com/bug?id=b68d24ad0de64bdba684
// status:0
...
{
  intptr_t res = 0;
  NONFAILING(memcpy((void*)0x20000080, "/dev/ttyS3\000", 11));
  res = syscall(__NR_openat, 0xffffffffffffff9cul, 0x20000080ul, 0ul, 0ul);
  if (res != -1)
    r[0] = res;
  NONFAILING(*(uint32_t*)0x20000100 = 0x18004);
  syscall(__NR_ioctl, r[0], 0x5418, 0x20000100ul);
  NONFAILING(memcpy((void*)0x20000040, "/dev/ttyS3\000", 11));
  res = syscall(__NR_openat, 0xffffffffffffff9cul, 0x20000040ul, 0x41ul, 0ul);
  if (res != -1)
    r[1] = res;
  NONFAILING(memcpy((void*)0x20000000, "/proc/meminfo\000", 14));
  res = syscall(__NR_openat, 0xffffffffffffff9cul, 0x20000000ul, 0ul, 0ul);
  if (res != -1)
    r[2] = res;
  syscall(__NR_sendfile, r[1], r[2], 0ul, 0x20000000005ul);
}
int main(void)
{
