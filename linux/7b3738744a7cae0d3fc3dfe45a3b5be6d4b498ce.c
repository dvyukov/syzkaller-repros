...
// https://syzkaller.appspot.com/bug?id=7b3738744a7cae0d3fc3dfe45a3b5be6d4b498ce
// status:open
...
void execute_one(void)
{
  ...
  *(uint64_t*)0x20000150 = 0x1a0;
  syz_read_part_table(0, 1, 0x20000140);
  syscall(__NR_openat, -1, 0, 0x26e1, 0);
  res = syscall(__NR_perf_event_open, 0, 0, -1, -1, 0);
  if (res != -1)
    r[0] = res;
  syscall(__NR_openat, r[0], 0, 0x200002, 0);
  syscall(__NR_getgid);
  res = syscall(__NR_memfd_create, 0, 3);
  if (res != -1)
    r[1] = res;
  syscall(__NR_write, r[1], 0, 0);
  syscall(__NR_fcntl, r[1], 0x409, 0);
  ...
  syscall(__NR_fcntl, r[2], 0x400, 0);
  syscall(__NR_pwrite64, r[1], 0, 0, 0);
  ...
  if (res != -1)
    r[3] = res;
  syscall(__NR_fcntl, -1, 0x409, 8);
  syscall(__NR_pwrite64, r[3], 0, 0, 0);
