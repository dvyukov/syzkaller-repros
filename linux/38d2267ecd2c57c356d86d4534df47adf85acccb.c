...
// https://syzkaller.appspot.com/bug?id=38d2267ecd2c57c356d86d4534df47adf85acccb
// status:fixed
...
void execute_call(int call)
{
  ...
  switch (call) {
  case 0:
    NONFAILING(memcpy((void*)0x20000000, "/dev/ptmx\000", 10));
    res = syscall(__NR_openat, 0xffffffffffffff9c, 0x20000000, 0xa801, 0);
    if (res != -1)
      r[0] = res;
    break;
  case 1:
    NONFAILING(*(uint32_t*)0x20000040 = r[0]);
    NONFAILING(*(uint16_t*)0x20000044 = 0x1040);
    NONFAILING(*(uint16_t*)0x20000046 = 0);
    syscall(__NR_ppoll, 0x20000040, 1, 0, 0, 0);
    break;
  case 2:
    NONFAILING(*(uint32_t*)0x20000080 = 7);
    NONFAILING(*(uint8_t*)0x20000084 = 0x6d);
    NONFAILING(*(uint16_t*)0x20000085 = 0);
    syscall(__NR_write, r[0], 0x20000080, 7);
    break;
  }
}
int main(void)
