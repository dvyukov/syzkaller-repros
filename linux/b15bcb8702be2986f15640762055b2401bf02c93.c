...
{
  long res = 0;
  syscall(__NR_clone, 0x802102001ffc, 0, 0x9999999999999999, 0x200000c0, -1);
  syscall(__NR_socket, 0xa, 0x805, 0x43);
  res = syscall(__NR_socketpair, 1, 2, 0, 0x20000180);
  if (res != -1)
    NONFAILING(r[0] = *(uint32_t*)0x20000184);
  syscall(__NR_ioctl, r[0], 0x8912, 0x400200);
  NONFAILING(memcpy((void*)0x200000c0, "/dev/ppp", 9));
  res = syscall(__NR_openat, 0xffffffffffffff9c, 0x200000c0, 0, 0);
  if (res != -1)
    r[1] = res;
  NONFAILING(*(uint32_t*)0x20000000 = 0);
  NONFAILING(*(uint64_t*)0x20000008 = 0x20000240);
  syscall(__NR_ioctl, -1, 0x800448d2, 0x20000000);
  syscall(__NR_ioctl, r[1], 0x40107446, 0x20000000);
  NONFAILING(*(uint32_t*)0x20000140 = 0xffffffb3);
  syscall(__NR_getsockopt, r[1], 0, 0x10, 0x20000300, 0x20000140);
  NONFAILING(memcpy((void*)0x20000280,
                    "\x15\x3f\x62\x34\x48\x8d\xd2\x5d\x76\x60\x70", 11));
  syscall(__NR_ioctl, -1, 0x8912, 0x20000280);
  NONFAILING(memcpy((void*)0x20000080, "oom_score_adj", 14));
  res = syz_open_procfs(0, 0x20000080);
  if (res != -1)
    r[2] = res;
  NONFAILING(*(uint64_t*)0x20000100 = 0x20000000);
  NONFAILING(*(uint64_t*)0x20000108 = 0);
  syscall(__NR_writev, r[2], 0x20000100, 1);
  ...
  NONFAILING(*(uint32_t*)0x20044010 = 0);
  syscall(__NR_timer_create, 0, 0x20044000, 0x20044000);
  NONFAILING(memcpy((void*)0x20000000, "/dev/vhost-vsock", 17));
  res = syscall(__NR_openat, 0xffffffffffffff9c, 0x20000000, 2, 0);
  if (res != -1)
    r[3] = res;
  NONFAILING(*(uint32_t*)0x20000040 = 0x4d2 + procid * 2);
  NONFAILING(*(uint32_t*)0x20000044 = 0);
  syscall(__NR_ioctl, r[3], 0x4008af60, 0x20000040);
  res = syscall(__NR_socket, 0x28, 1, 0);
  if (res != -1)
    r[4] = res;
  NONFAILING(*(uint16_t*)0x20000200 = 0x28);
  NONFAILING(*(uint16_t*)0x20000202 = 0);
  NONFAILING(*(uint32_t*)0x20000204 = 0);
  NONFAILING(*(uint32_t*)0x20000208 = 0x4d2 + procid * 2);
  NONFAILING(*(uint32_t*)0x2000020c = 0);
  syscall(__NR_connect, r[4], 0x20000200, 0x10);
  NONFAILING(*(uint64_t*)0x2006b000 = 0);
  NONFAILING(*(uint64_t*)0x2006b008 = 8);
  NONFAILING(*(uint64_t*)0x2006b010 = 0);
  NONFAILING(*(uint64_t*)0x2006b018 = 0x1c9c380);
  syscall(__NR_timer_settime, 0, 0, 0x2006b000, 0x20040000);
}
int main(void)
{
