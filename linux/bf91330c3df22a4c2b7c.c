...
// https://syzkaller.appspot.com/bug?id=bf91330c3df22a4c2b7c
// status:0
...
void loop(void)
{
  ...
  if (res != -1)
    r[0] = res;
  res = syscall(__NR_io_setup, 4, 0x20000040ul);
  if (res != -1)
    NONFAILING(r[1] = *(uint64_t*)0x20000040);
  NONFAILING(*(uint64_t*)0x20001240 = 0x20001200);
  NONFAILING(*(uint64_t*)0x20001200 = 0);
  NONFAILING(*(uint32_t*)0x20001208 = 0);
  NONFAILING(*(uint32_t*)0x2000120c = 0);
  NONFAILING(*(uint16_t*)0x20001210 = 5);
  NONFAILING(*(uint16_t*)0x20001212 = 0);
  NONFAILING(*(uint32_t*)0x20001214 = r[0]);
  ...
  NONFAILING(*(uint32_t*)0x20001238 = 0);
  NONFAILING(*(uint32_t*)0x2000123c = -1);
  syscall(__NR_io_submit, r[1], 1ul, 0x20001240ul);
  syscall(__NR_socket, 0x1dul, 3ul, 1);
  syscall(__NR_unshare, 0x62040400ul);
  close_fds();
}
int main(void)
{
