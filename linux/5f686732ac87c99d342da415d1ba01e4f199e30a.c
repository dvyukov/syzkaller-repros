...
// https://syzkaller.appspot.com/bug?id=5f686732ac87c99d342da415d1ba01e4f199e30a
// status:open
...
void execute_one(void)
{
  ...
  syscall(__NR_ioctl, -1, 0x401054d5ul, 0ul);
  ...
  *(uint64_t*)0x20000720 = 0;
  ...
  STORE_BY_BITMASK(uint64_t, , 0x20000728, 1, 7, 1);
  STORE_BY_BITMASK(uint64_t, , 0x20000728, 0, 8, 1);
  STORE_BY_BITMASK(uint64_t, , 0x20000728, 1, 9, 1);
  STORE_BY_BITMASK(uint64_t, , 0x20000728, 0, 10, 1);
  STORE_BY_BITMASK(uint64_t, , 0x20000728, 1, 11, 1);
  STORE_BY_BITMASK(uint64_t, , 0x20000728, 1, 12, 1);
  ...
  STORE_BY_BITMASK(uint64_t, , 0x20000728, 1, 24, 1);
  ...
  *(uint16_t*)0x2000076e = 0;
  syscall(__NR_perf_event_open, 0x20000700ul, 0, 0ul, r[0], 0ul);
  memcpy((void*)0x20000000, "net/ipv6_route\000", 15);
  res = syz_open_procfs(-1, 0x20000000);
  if (res != -1)
    r[1] = res;
  memcpy((void*)0x20000080, "/sys/kernel/debug/bluetooth/6lowpan_enable\000",
         43);
  res = syscall(__NR_openat, 0xffffffffffffff9cul, 0x20000080ul, 2ul, 0ul);
  if (res != -1)
    r[2] = res;
  syscall(__NR_sendfile, r[2], r[1], 0ul, 0x5000ul);
  res = syscall(__NR_getsockopt, -1, 0x84ul, 0x14ul, 0x20000000ul, 0ul);
  if (res != -1)
    r[3] = *(uint32_t*)0x20000000;
  *(uint32_t*)0x20000780 = r[3];
  *(uint16_t*)0x20000784 = 0xa;
  *(uint16_t*)0x20000786 = htobe16(0x4e22);
  *(uint32_t*)0x20000788 = htobe32(0x8000);
  memcpy((void*)0x2000078c,
         "\x30\x3f\x4a\x4a\x85\x61\xf9\x8e\x41\xf4\x75\xd4\x74\x7b\x3e\x79",
         16);
  *(uint32_t*)0x2000079c = 0x240000;
  syscall(__NR_getsockopt, -1, 0x84ul, 6ul, 0x20000780ul, 0ul);
  syscall(__NR_getsockopt, r[1], 0x84ul, 0x7bul, 0ul, 0ul);
  syscall(__NR_setsockopt, -1, 0x84ul, 9ul, 0ul, 0ul);
  syscall(__NR_openat, r[0], 0ul, 2ul, 0ul);
