...
void loop(void)
{
  ...
  NONFAILING(
      memcpy((void*)0x20000100,
             "comm\000(\266u\206S\312\346\022n\241\200u\272|\270\352I\315\210,"
             "\252\tE\020x\374\366\274\223\212\005\000\000\000\002rI\342\325"
             "\307\333l+\215\235a\330\207",
             53));
  res = syz_open_procfs(-1, 0x20000100);
  if (res != -1)
    r[0] = res;
  NONFAILING(memcpy((void*)0x20000300, "/dev/ptmx\000", 10));
  res = syscall(__NR_openat, 0xffffffffffffff9cul, 0x20000300ul, 0ul, 0ul);
  if (res != -1)
    r[1] = res;
  NONFAILING(*(uint32_t*)0x20000080 = 1);
  syscall(__NR_ioctl, r[1], 0x5423ul, 0x20000080ul);
  res = syscall(__NR_dup3, r[1], r[0], 0ul);
  if (res != -1)
    r[2] = res;
  syscall(__NR_ioctl, r[2], 0x8926ul, 0x710000ul);
  close_fds();
}
int main(void)
{
