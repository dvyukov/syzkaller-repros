...
// https://syzkaller.appspot.com/bug?id=e493e79b0a398fb608190237e9f2d399fdbe8580
// status:dup
...
int main(void)
{
  ...
  *(uint64_t*)0x20000154 = 0x200000c0;
  *(uint8_t*)0x200000c0 = 0;
  *(uint8_t*)0x200000c1 = 0x22;
  *(uint32_t*)0x200000c2 = 0x22;
  STORE_BY_BITMASK(uint8_t, , 0x200000c6, 0, 0, 2);
  STORE_BY_BITMASK(uint8_t, , 0x200000c6, 0, 2, 2);
  STORE_BY_BITMASK(uint8_t, , 0x200000c6, 0xb, 4, 4);
  memcpy((void*)0x200000c7, "]z", 2);
  STORE_BY_BITMASK(uint8_t, , 0x200000c9, 3, 0, 2);
  STORE_BY_BITMASK(uint8_t, , 0x200000c9, 2, 2, 2);
  STORE_BY_BITMASK(uint8_t, , 0x200000c9, 0, 4, 4);
  memcpy((void*)0x200000ca, "\xc4\xeb\x48\xfd", 4);
  STORE_BY_BITMASK(uint8_t, , 0x200000ce, 2, 0, 2);
  STORE_BY_BITMASK(uint8_t, , 0x200000ce, 2, 2, 2);
  STORE_BY_BITMASK(uint8_t, , 0x200000ce, 0, 4, 4);
  memcpy((void*)0x200000cf, "\x8c\x93", 2);
  STORE_BY_BITMASK(uint8_t, , 0x200000d1, 3, 0, 2);
  STORE_BY_BITMASK(uint8_t, , 0x200000d1, 2, 2, 2);
  STORE_BY_BITMASK(uint8_t, , 0x200000d1, 0, 4, 4);
  memcpy((void*)0x200000d2, "\xaf\x9d\xb9\xb5", 4);
  STORE_BY_BITMASK(uint8_t, , 0x200000d6, 0, 0, 2);
  STORE_BY_BITMASK(uint8_t, , 0x200000d6, 2, 2, 2);
  STORE_BY_BITMASK(uint8_t, , 0x200000d6, 0, 4, 4);
  STORE_BY_BITMASK(uint8_t, , 0x200000d7, 3, 0, 2);
  STORE_BY_BITMASK(uint8_t, , 0x200000d7, 1, 2, 2);
  STORE_BY_BITMASK(uint8_t, , 0x200000d7, 0, 4, 4);
  memcpy((void*)0x200000d8, "\xd9\xc6\x8a\x50", 4);
  STORE_BY_BITMASK(uint8_t, , 0x200000dc, 3, 0, 2);
  STORE_BY_BITMASK(uint8_t, , 0x200000dc, 2, 2, 2);
  STORE_BY_BITMASK(uint8_t, , 0x200000dc, 0, 4, 4);
  memcpy((void*)0x200000dd, "\x18\xc1\x18\x7b", 4);
  STORE_BY_BITMASK(uint8_t, , 0x200000e1, 3, 0, 2);
  STORE_BY_BITMASK(uint8_t, , 0x200000e1, 0, 2, 2);
  STORE_BY_BITMASK(uint8_t, , 0x200000e1, 0, 4, 4);
  memcpy((void*)0x200000e2, "\xaf\x64\x4c\x25", 4);
  *(uint64_t*)0x2000015c = 0;
  *(uint32_t*)0x20000380 = 0x26;
  *(uint64_t*)0x20000384 = 0;
  *(uint64_t*)0x2000038c = 0;
  *(uint64_t*)0x20000394 = 0;
  *(uint64_t*)0x2000039c = 0;
  *(uint64_t*)0x200003a4 = 0;
  syz_usb_control_io(r[0], 0x20000140, 0x20000380);
  memcpy((void*)0x200011c0, "/dev/usb/hiddev#\000", 17);
  res = syz_open_dev(0x200011c0, 0, 0);
  if (res != -1)
    r[1] = res;
  *(uint32_t*)0x20000080 = 3;
  *(uint32_t*)0x20000084 = 0x100;
  *(uint32_t*)0x20000088 = 9;
  syscall(__NR_ioctl, r[1], 0x400c4808, 0x20000080);
  return 0;
}
