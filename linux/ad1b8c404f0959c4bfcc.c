...
// https://syzkaller.appspot.com/bug?id=ad1b8c404f0959c4bfcc
// status:0
...
{
  intptr_t res = 0;
  syscall(__NR_mbind, 0x20000000ul, 0x600000ul, 1ul, 0ul, 0ul, 0ul);
  ...
  if (res != -1)
    r[0] = res;
  memcpy((void*)0x200006c0, "cgroup.procs\000", 13);
  res = syscall(__NR_openat, r[0], 0x200006c0ul, 2ul, 0ul);
  if (res != -1)
    r[1] = res;
  sprintf((char*)0x20000100, "0x%016llx", (long long)0);
  syscall(__NR_write, r[1], 0x20000100ul, 0x12ul);
}
int main(void)
{
