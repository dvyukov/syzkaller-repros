...
// https://syzkaller.appspot.com/bug?id=2bb1968c6e2f8782c289
// status:0
...
void loop(void)
{
  ...
  syscall(__NR_ioctl, r[0], 0x89a0, 0ul);
  *(uint8_t*)0x20000040 = 4;
  *(uint8_t*)0x20000041 = 0x46;
  *(uint8_t*)0x20000042 = 4;
  *(uint8_t*)0x20000043 = 0;
  *(uint16_t*)0x20000044 = 0;
  *(uint8_t*)0x20000046 = 0;
  syz_emit_vhci(0x20000040, 7);
  res = syscall(__NR_socket, 0x1ful, 3ul, 6);
  if (res != -1)
    r[1] = res;
  syscall(__NR_ioctl, r[1], 0x891b, 0ul);
  syscall(__NR_getsockopt, r[1], 0x112, 0xc, 0ul, 0ul);
  res = -1;
  res = syz_init_net_socket(0x1f, 5, 2);
  if (res != -1)
    r[2] = res;
  syscall(__NR_setsockopt, r[2], 1, 0x15, 0ul, 0ul);
}
int main(void)
{
