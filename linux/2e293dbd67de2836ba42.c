...
// https://syzkaller.appspot.com/bug?id=2e293dbd67de2836ba42
// status:6
...
void execute_one(void)
{
  ...
  if (res != -1)
    r[0] = res;
  syscall(__NR_setsockopt, r[0], 6, 2, 0ul, 0ul);
  NONFAILING(*(uint16_t*)0x20000440 = 2);
  NONFAILING(*(uint16_t*)0x20000442 = htobe16(0x4e23));
  NONFAILING(*(uint32_t*)0x20000444 = htobe32(-1));
  syscall(__NR_bind, r[0], 0x20000440ul, 0x10ul);
  NONFAILING(*(uint16_t*)0x20000480 = 0);
  NONFAILING(*(uint64_t*)0x20000488 = 0);
  syscall(__NR_setsockopt, r[0], 1, 0x1a, 0x20000480ul, 0x10ul);
  ...
  syscall(__NR_sendto, r[0], 0ul, 0ul, 0x200007fdul, 0x20e68000ul, 0x10ul);
  NONFAILING(memcpy((void*)0x20000100, "bbr\000", 4));
  syscall(__NR_setsockopt, r[0], 6, 0xd, 0x20000100ul, 4ul);
  ...
  NONFAILING(*(uint32_t*)0x20000600 = 0x1801);
  syscall(__NR_setsockopt, r[0], 1, 8, 0x20000600ul, 4ul);
  syscall(__NR_sendmmsg, r[0], 0ul, 0ul, 0ul);
  NONFAILING(memcpy((void*)0x200012c0,
                    "\x0c\x26\x8a\x92\x7f\x1f\x65\x88\xb9\x67\x48\x12\x41\xba"
                    "\x78\x60\xfc\xfa\xf6\x5a\xc6\x18\xde\xd8\x97\x48\x95\xab"
                    "\xea\xf4\xb4\x83\x4f\xf9\x22\xb3\xf1\xe0\xb0\x2b\xd6\x7a"
                    "\xa0\x30\x59\xbc\xec\xc7\xa9\x54\x25\xa3\xa0\x7e\x75\x80"
                    "\x44\xab\x4e\xa6\xf7\xae\x55\xd8\x8f\xec\xf9\x0b\x1a\x75"
                    "\x11\xbf\x74\x6b\xec\x66\xba",
                    77));
  syscall(__NR_sendto, r[0], 0x200012c0ul, 0x1534ul, 0x11ul, 0ul, 0x27ul);
}
int main(void)
{
