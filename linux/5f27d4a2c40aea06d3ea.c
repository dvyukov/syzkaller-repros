...
// https://syzkaller.appspot.com/bug?id=5f27d4a2c40aea06d3ea
// status:0
...
int main(void)
{
  ...
  *(uint32_t*)0x20001218 = 0;
  ...
  memcpy((void*)0x20000100,
         "\x12\x01\x00\x00\x8f\x9a\xb0\x40\x81\x07\x00\x01\x00\x01\x01\x02\x03"
         "\x01\x09\x02\xba\x03\x01\x00\x00\x00\x00\x09\x04\x7b\x00\x0f\x5c\x87"
         "\x9b\x7f\xff\xff\xff\xff\xff\xff\xff\x08\x60\x09\x05\x00\x00\x10\x00"
         "\x01\x81\x03\x21\x05\x0a\x10\x08\x00\x08\x1c\x01\x09\x05",
         65);
  ...
  if (res != -1)
    r[1] = res;
  syscall(__NR_ioctl, r[1], 0x5452, 0x20000140ul);
  memcpy((void*)0x20001140, "/dev/input/event#\000", 18);
  res = -1;
  ...
  syscall(__NR_write, r[2], 0x20000040ul, 0x35000ul);
  return 0;
}
