...
// https://syzkaller.appspot.com/bug?id=eb4579cce1e26940e2e1
// status:0
...
{
  intptr_t res = 0;
  syscall(__NR_mmap, 0x20011000ul, 0x3000ul, 0x200000cul, 0x10032ul, -1, 0ul);
  res = syscall(__NR_dup2, -1, -1);
  if (res != -1)
    r[0] = res;
  NONFAILING(memcpy((void*)0x20000440, "./file0\000", 8));
  syscall(__NR_open, 0x20000440ul, 0x110000141042ul, 0ul);
  syscall(__NR_clone, 0x4007fcul, 0ul, 0x9999999999999999ul, 0ul, -1ul);
  res = syscall(__NR_userfaultfd, 0ul);
  if (res != -1)
    r[1] = res;
  NONFAILING(*(uint64_t*)0x20000380 = 0xaa);
  NONFAILING(*(uint64_t*)0x20000388 = 0);
  NONFAILING(*(uint64_t*)0x20000390 = 0);
  syscall(__NR_ioctl, r[1], 0xc018aa3f, 0x20000380ul);
  syscall(__NR_ioctl, r[0], 0x80047453, 0ul);
  NONFAILING(*(uint64_t*)0x20000080 = 0x20909000);
  NONFAILING(*(uint64_t*)0x20000088 = 0x4000);
  ...
  if (res != -1)
    r[2] = res;
  syscall(__NR_write, r[2], 0x20000040ul, 0x208e24bul);
}
int main(void)
{
