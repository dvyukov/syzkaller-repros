...
// https://syzkaller.appspot.com/bug?id=cd2a427abfbf4e92ead7ada51dc4e48fe160bc95
// status:open
...
int main(void)
{
  ...
  if (res != -1)
    r[2] = res;
  NONFAILING(*(uint64_t*)0x20000540 = 0x40);
  NONFAILING(*(uint64_t*)0x20000548 = 0x200004c0);
  NONFAILING(memcpy((void*)0x200004c0,
                    "\x48\xb8\x00\x80\x00\x00\x00\x00\x00\x00\x0f\x23\xd8\x0f"
                    "\x21\xf8\x35\x80\x00\x00\x80\x0f\x23\xf8\x0f\x20\xd8\x35"
                    "\x08\x00\x00\x00\x0f\x22\xd8\x66\x0f\x38\x81\x1a\x0f\x20"
                    "\xd8\x35\x20\x00\x00\x00\x0f\x22\xd8\x42\x0f\x21\x8f\xc4"
                    "\xe3\x7d\x05\x36\x00\x66\xba\xa0\x00\x66\xed\x3e\x46\x0f"
                    "\x00\xd6\x26\x40\x0f\x01\xc5\xc7\x44\x24\x00\x27\x01\x00"
                    "\x00\xc7\x44\x24\x02\x8d\x00\x00\x00\xff\x2c\x24",
                    96));
  NONFAILING(*(uint64_t*)0x20000550 = 0x60);
  syz_kvm_setup_cpu(-1, r[2], 0x20000000, 0x20000540, 1, 0, 0, 0);
  NONFAILING(memcpy((void*)0x20000000, "/dev/video35\000", 13));
  res = syscall(__NR_openat, 0xffffffffffffff9c, 0x20000000, 2, 0);
  if (res != -1)
    r[3] = res;
  NONFAILING(*(uint32_t*)0x20000040 = 0x800);
  NONFAILING(*(uint32_t*)0x20000044 = 7);
  NONFAILING(*(uint32_t*)0x20000048 = 0x212);
  NONFAILING(*(uint32_t*)0x2000004c = 0x40);
  NONFAILING(*(uint32_t*)0x20000050 = 0xd4b);
  NONFAILING(*(uint32_t*)0x20000054 = 0xffffff00);
  NONFAILING(*(uint32_t*)0x20000058 = 2);
  ...
  syscall(__NR_ioctl, r[3], 0xc0405665, 0x20000040);
  syz_kvm_setup_cpu(r[1], -1, 0x20000000, 0, 0xfffffffffffffd18, 0, 0, 0);
