...
// https://syzkaller.appspot.com/bug?id=a8badd8dc31a2e7d4fde5511149065e7fde06cc1
// status:invalid
...
{
  long res = 0;
  NONFAILING(memcpy((void*)0x20000240, "/dev/ion", 9));
  res = syscall(__NR_openat, 0xffffffffffffff9c, 0x20000240, 0, 0);
  if (res != -1)
    r[0] = res;
  NONFAILING(*(uint64_t*)0x20000500 = 0x10000009);
  NONFAILING(*(uint32_t*)0x20000508 = -1);
  NONFAILING(*(uint32_t*)0x2000050c = 0);
  NONFAILING(*(uint32_t*)0x20000510 = -1);
  NONFAILING(*(uint32_t*)0x20000514 = 0);
  res = syscall(__NR_ioctl, r[0], 0xc0184900, 0x20000500);
  if (res != -1)
    NONFAILING(r[1] = *(uint32_t*)0x20000510);
  write_file("/sys/kernel/debug/failslab/ignore-gfp-wait", "N");
  write_file("/sys/kernel/debug/fail_futex/ignore-private", "N");
  inject_fault(5);
  syscall(__NR_mmap, 0x20ffd000, 0x3060, 0, 0x4000812, r[1], 0);
}
