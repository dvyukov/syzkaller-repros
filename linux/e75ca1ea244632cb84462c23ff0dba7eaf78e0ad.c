...
// https://syzkaller.appspot.com/bug?id=e75ca1ea244632cb84462c23ff0dba7eaf78e0ad
// status:open
...
int main(void)
{
  ...
  intptr_t res = 0;
  NONFAILING(memcpy((void*)0x20000400, "/dev/kvm\000", 9));
  ...
  res = syscall(__NR_getsockopt, r[2], 1, 0x11, 0x20000240, 0x20000280);
  if (res != -1)
  ...
  if (res != -1)
    r[4] = res;
  NONFAILING(*(uint64_t*)0x200000c0 = 0x40);
  NONFAILING(*(uint64_t*)0x200000c8 = 0x200002c0);
  NONFAILING(memcpy((void*)0x200002c0,
                    "\xb9\x59\x08\x00\x00\xb8\x68\xcd\x67\x8d\xba\x00\x00\x00"
                    "\x00\x0f\x30\x0f\x01\xb0\x00\x00\x00\x00\x48\xb8\x0c\x00"
                    "\x00\x00\x00\x00\x00\x00\x0f\x23\xd8\x0f\x21\xf8\x35\x80"
                    "\x00\x00\xa0\x0f\x23\xf8\x66\x44\x0f\x38\x81\xbf\x10\xf6"
                    "\x4b\x39\x66\xb8\xc3\x00\x8e\xe8\xb9\x80\x00\x00\xc0\x0f"
                    "\x32\x35\x00\x04\x00\x00\x0f\x30\xc4\x43\x89\x5d\xe0\x00"
                    "\x48\x0f\xc7\x28\x4a\x0f\xc7\x2c\xa9\xc7\x44\x24\x00\x9d"
                    "\x00\x00\x00\xc7\x44\x24\x02\xcd\x00\x00\x00\xff\x2c\x24",
                    112));
  NONFAILING(*(uint64_t*)0x200000d0 = 0x70);
  NONFAILING(*(uint64_t*)0x20000080 = 7);
  NONFAILING(*(uint64_t*)0x20000088 = 0);
  syz_kvm_setup_cpu(-1, r[4], 0x20000000, 0x200000c0, 1, 8, 0x20000080, 1);
  syz_kvm_setup_cpu(r[1], -1, 0x20000000, 0, 0, 0, 0, 0);
  NONFAILING(memcpy((void*)0x20000000, "./file0\000", 8));
  syscall(__NR_lsetxattr, 0x20000000, 0, 0, 0, 0);
