...
// https://syzkaller.appspot.com/bug?id=2a7156e11dc199bdbd8a
// status:6
...
int main(void)
{
  ...
  res = syscall(__NR_open, 0x20000240ul, 0x141042ul, 0ul);
  if (res != -1)
    r[2] = res;
  syscall(__NR_mmap, 0x20005000ul, 0x1000ul, 0ul, 0x2812ul, r[2], 0ul);
  res = syscall(__NR_ioctl, r[1], 0xae41, 0ul);
  if (res != -1)
    r[3] = res;
  syz_kvm_setup_cpu(r[1], -1, 0x20000000, 0, 0, 0, 0, 0);
  syz_kvm_setup_cpu(-1, r[3], 0x20000000, 0, 0, 0x42, 0, 0);
